...
class App {
    ...
    //Enums
    editMode = EditorTypes.DrawWalls;
    algorithmType = AlgorithmTypes.Dijkstra;
    //Algorithms
    dijkstra = new Dijkstra();
    aStar = new AStar();
    bfsdfs = new BFSDFS();
    greedy = new GreedyBestFirst();
    bi = new Bidirectional();
    //Bee
    bee = null;
    //Terminal
    terminal = null;
    dormantMessage = "Press \"Animate Path\" to begin animation.";
    animatingMessage = "ANIMATING PATH! Please wait...";
    failedMessage = "FAILED! Could not reach target.";
    succeededMessage = "SUCCEEDED! Target was reached.";
    //#endregion
    //#region Constructor
    constructor() {
        //Debug
        ...
        for (let y = 0; y < ROW_COUNT; y++) {
            for (let x = 0; x < COL_COUNT; x++) {
                ...
                //Cardinals             
                if (y > 0)
                    node.neighbours.push(this.nodes[y - 1][x]); //North
                if (x < COL_COUNT - 1)
                    node.neighbours.push(this.nodes[y][x + 1]); //East
                if (y < ROW_COUNT - 1)
                    node.neighbours.push(this.nodes[y + 1][x]); //South
                if (x > 0)
                    node.neighbours.push(this.nodes[y][x - 1]); //West
                //Diagonals
                if (y > 0 && x > 0)
    ...
    //#endregion
    //#region StartMethods
    buildNodeMap = (container, nodes, showGrid) => {
        nodes.map(row => {
    ...
    animatePath = () => {
        ...
        //Animate path
        this.nodeController.AnimatePath(pathNodes, _shortPathNodes, this.showGrid, this.showResult, () => { this.isAnimatingPath = false; });
    };
    getAlgorithmPath = () => {
    ...
    setClass = (node, showGrid) => {
        const { isStartNode, isStartNodePreview, isEndNode, isEndNodePreview, isError, isWall, wall_bgx, isWeighted: weighted } = node;
        let string = `node`;
        if (isError) {
            string += ' error';
        }
