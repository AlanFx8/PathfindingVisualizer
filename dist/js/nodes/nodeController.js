export default class NodeController {
    ...
    ResetWeights = (nodes, classSetter, showGrid) => {
        for (let row of nodes) {
            for (let node of row) {
                node.WeightOff();
                const newClass = classSetter(node, showGrid);
    ...
    AnimatePath = (visitedList, shortestPath, showGrid, showResult, callback) => {
    ...
    //#endregion
    //#region Private
    _animateShortestPath = (path, index, showGrid, showResult, callback) => {
        if (path.length <= 1) {
